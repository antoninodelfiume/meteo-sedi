        ...
        >
          <Box>
            ...
            <Typography color="text.secondary">
              Percepita{" "}
              {formatTemperature(forecast.current.apparentTemperature, unit)}
            </Typography>
          </Box>
          ...
          {forecast.daily.map((day) => (
            <Paper key={day.date} variant="outlined" sx={{ p: 2 }}>
              ...
              <Typography color="text.secondary" sx={{ mt: 1 }}>
                {formatTemperature(day.minimumTemperature, unit)} /{" "}
                {formatTemperature(day.maximumTemperature, unit)}
              </Typography>
            </Paper>
          ))}
        </Box>
...
function ForecastContent({
  ...
  if (state.status === "idle") {
    return (
      <Alert icon={<CloudOutlinedIcon fontSize="inherit" />} severity="info">
        <AlertTitle>Dati meteo da collegare</AlertTitle>
        Completa i TODO del brief per caricare le condizioni della sede
        selezionata.
      </Alert>
    );
  }
  ...
  if (state.status === "loading") {
    return (
      <Stack role="status" aria-label="Caricamento dati meteo" spacing={2}>
  ...
  return (
    <ForecastSummary forecast={state.data} unit={unit} onRefresh={refresh} />
  );
}
...
export function WeatherDashboard({ api }: { api: WeatherApi }) {
  ...
  const { retry, refresh, ...state } = useForecast(
    api,
    selectedSite,
    temperatureUnit,
  );
              ...
              >
                {officeSites.map((site) => (
                  <Button
                    key={site.id}
                    variant={
                      selectedSiteId === site.id ? "contained" : "outlined"
                    }
